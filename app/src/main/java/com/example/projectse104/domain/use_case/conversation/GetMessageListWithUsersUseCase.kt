...
class GetMessageListWithUsersUseCase @Inject constructor(
                ...
                for (conversation in conversationList) {
                    when (val lastMessage = messageRepository.getLastMessage(conversation.id)) {
                        is Response.Success -> {
                            ...
                            val senderId = lastMessage.data?.senderId ?: continue

                            val senderName =
                                when (val senderResponse = userRepository.getUser(senderId)) {
                                    is Response.Success -> {
